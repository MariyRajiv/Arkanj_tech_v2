  ...
  <footer className="bg-slate-900 pt-32 pb-12 text-slate-400 relative overflow-hidden">
    ...
    <div className="max-w-7xl mx-auto px-6 relative z-10">
      <div className="grid md:grid-cols-4 gap-16 mb-24">
        <div className="col-span-1 md:col-span-1 text-left">
          ...
          <div className="flex gap-4">
            {[
              ...
              { icon: <Linkedin className="w-5 h-5" />, url: 'https://www.linkedin.com/in/admin-arkanj-08b18b402', label: 'LinkedIn' }
            ].map((social, i) => (
              <motion.a 
                key={i}
                href={social.url}
        ...
        <div className="text-left">
          ...
          <ul className="space-y-5 text-lg font-medium">
            {['German Language', 'AI & Automation', 'FinTech Solutions', 'DeepTech R&D'].map((item) => (
              <li key={item}>
                <button onClick={() => setPage('services')} className="hover:text-blue-400 cursor-pointer transition-all hover:translate-x-2 inline-block w-full text-left">
                  {item}
                </button>
              </li>
            ))}
        ...
        <div className="text-left">
          ...
          <ul className="space-y-5 text-lg font-medium">
            {[
              ...
              <li key={link.name}>
                <button onClick={() => setPage(link.page as Page)} className="hover:text-blue-400 cursor-pointer transition-all hover:translate-x-2 inline-block w-full text-left">
                  {link.name}
                </button>
              </li>
            ))}
        ...
        <div className="text-left">
          ...
          <div className="space-y-6 text-lg font-medium">
            <button className="flex gap-4 cursor-pointer group w-full text-left" onClick={() => setPage('contact')}>
            ...
            <button className="flex gap-4 cursor-pointer group w-full text-left" onClick={() => window.location.href = 'mailto:superadmin@arkanj.tech'}>
              <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-blue-600/20 transition-colors">
      ...
      <div className="border-t border-white/5 pt-12 flex flex-col md:flex-row justify-between items-center gap-8 text-[11px] uppercase tracking-[0.3em] font-black">
        <p className="text-slate-400">© 2026 Arkanj Tech Solutions. All rights reserved.</p>
        <div className="flex gap-12">
          <button onClick={() => setPage('contact')} className="hover:text-white cursor-pointer transition-colors">Privacy Policy</button>
          <button onClick={() => setPage('contact')} className="hover:text-white cursor-pointer transition-colors">Terms of Service</button>
        </div>
      </div>
    </div>
  </footer>
